...
// import dropdown from '../assests/arrow-down-sign-to-navigate (1).png'
...
export default function Navbar() {
  ...
  const [menuopen,setmenuopen] = useState(false)
  const [query, setQuery] = useState('');
  const [userlog,setuserlogo] = useState(false)
  const { results, setResults } = useContext(MyListContext);
  console.log(results)
  var handlesearchdisplay = () => {
    setmodalopen(true)
    ...
    return (
      <div className='navbar' ref={navbarref} >
        ...
        <div className='navbarright'>
          ...
          <div><img className='userstyle' src={userlogo} alt='userlogo' onClick={handlelogoopen} style={{ width: '20px' }} /></div>
          {userlog && <div className='userlogostyle'>
           <Subscription/>
          </div>}
          <div className='dropdown' >
            <div className='dropdown2'> <img src={dropdown} alt='dropdownmenu' style={{ width: '20px' }} /></div>
